    ...
    public class CCIContext : IContext
    {
        ...
        public void View(frmExplorer f, int view, int[] values)
        {
            ...
            switch ((CCIView)view)
            {
                ...
                case CCIView.NCCH:
                    ...
                    var info = TitleInfo.Resolve(cxis[i].ProductCode,cxis[i].MakerCode);
                    ...
                    f.AddListItem(0x000, 0x100, "RSA-2048 signature of the NCCH header [SHA-256]", cxis[i].NCCHHeaderSignature, 1);

                    f.AddListItem(0x100, 4, "Magic ID, always 'NCCH'", cxis[i].MagicID, 2);
                    f.AddListItem(0x104, 4, "Content size [medias]", cxis[i].CXISize, 2);
                    f.AddListItem(0x108, 8, "Partition ID", cxis[i].PartitionID, 2);
                    f.AddListItem(0x110, 2, "Maker Code", cxis[i].MakerCode, 2);
                    f.AddListItem(0x112, 2, "Version", cxis[i].Version, 2);
                    f.AddListItem(0x118, 8, "Program ID", cxis[i].ProgramID, 2);
                    f.AddListItem(0x120, 1, "Temp Flag", cxis[i].TempFlag, 2);
                    f.AddListItem(0x150, 0x10, "Product Code", cxis[i].ProductCode, 2);
                    f.AddListItem(0x160, 0x20, "Extended Header Hash", cxis[i].ExtendedHeaderHash, 2);
                    f.AddListItem(0x180, 4, "Extended header size", cxis[i].ExtendedHeaderSize, 2);
                    f.AddListItem(0x188, 8, "Flags", cxis[i].Flags, 2);
                    f.AddListItem(0x190, 4, "Plain region offset [medias]", cxis[i].PlainRegionOffset, 2);
                    f.AddListItem(0x194, 4, "Plain region size [medias]", cxis[i].PlainRegionSize, 2);
                    f.AddListItem(0x1A0, 4, "ExeFS offset [medias]", cxis[i].ExeFSOffset, 2);
                    f.AddListItem(0x1A4, 4, "ExeFS size [medias]", cxis[i].ExeFSSize, 2);
                    f.AddListItem(0x1A8, 4, "ExeFS hash region size [medias]", cxis[i].ExeFSHashRegionSize, 2);
                    f.AddListItem(0x1B0, 4, "RomFS offset [medias]", cxis[i].RomFSOffset, 2);
                    f.AddListItem(0x1B4, 4, "RomFS size [medias]", cxis[i].RomFSSize, 2);
                    f.AddListItem(0x1B8, 4, "RomFS hash region size [medias]", cxis[i].RomFSHashRegionSize, 2);
                    f.AddListItem(0x1C0, 0x20, "ExeFS superblock hash", cxis[i].ExeFSSuperBlockhash, 2);
                    f.AddListItem(0x1E0, 0x20, "RomFS superblock hash", cxis[i].RomFSSuperBlockhash, 2);
                    break;
                case CCIView.NCCHPlainRegion:
        ...
        public TreeNode GetExplorerTopNode()
        {
            ...
            for (var i = 0; i < cxis.Length; i++) //ADD CXIs
                if (cxis[i].CXISize > 0)
                {
                    tNode.Nodes.Add("NCCH " + i + " (" + (new String(cxis[i].ProductCode)).Substring(0, 10) + ")").Tag = TreeViewContextTag.Create(this,(int)CCIView.NCCH,new[] { i } );
                    if (cxis[i].PlainRegionSize > 0) //Add PlainRegions
                        tNode.Nodes[tNode.Nodes.Count - 1].Nodes.Add("PlainRegion").Tag = TreeViewContextTag.Create(this,(int)CCIView.NCCHPlainRegion,new[] { i });
                }
            return tNode;
        }
        ...
        public TreeNode GetFileSystemTopNode()
        {
            ...
            for (var i = 0; i < 2; i++)
            {
                ...
                ulong partitionOffset = (i == 0 ? cci.FirstNCCHOffset : i == 1 ? cci.SecondNCCHOffset : cci.ThirdNCCHOffset);
                if (cxis[i].ExeFSSize > 0)
                    tNode.Nodes.Add(new TreeNode(
                        TreeListView.TreeListViewControl.CreateMultiColumnNodeText(
                            ...
                            StringUtil.ToHexString(6, partitionOffset * 0x200 + cxis[i].ExeFSOffset * 0x200)
                            )) {Tag = cxis[i]});
                ...
                tNode.Nodes.Add(new TreeNode(
                        TreeListView.TreeListViewControl.CreateMultiColumnNodeText(
                            ...
                            StringUtil.ToHexString(6, partitionOffset * 0x200 + cxis[i].RomFSOffset * 0x200)
                            )) { Tag = cxis[i] });
            }
            return tNode;
        }
